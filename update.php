	...
	//echo $fname." - ".$lname." - ".$csrf_token;
      ...
      if(!empty($fname) && !empty($lname) && !empty($csrf_token))
	  {
        if(Token::check_token($csrf_token))
		{
          ...
          $display_messsge = "<p class=\"text-success\"  style='background-color:#fff;padding:10px;'><strong>".$messsge."</strong></p>";
        }
        else{
          $messsge = "Invalid token can not be update. Sorry!!!!";
          $display_messsge= "<p class=\"text-danger\"  style='background-color:#fff;padding:10px;'><strong>".$messsge."</strong></p>";
        }
      }
      else{
